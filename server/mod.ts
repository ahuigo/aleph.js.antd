...
function isUmiIndex(pathname: string) {
  if(pathname=="/") return true
  for (const p of ['/mauth/']) {
...
export function serve(options: ServerOptions = {}) {
  ...
  const handler = async (req: Request, connInfo: ConnInfo): Promise<Response> => {
    const { pathname:tmppath, searchParams } = new URL(req.url);
    //hack
    const pathname = isUmiIndex(tmppath)?'/umi.html':tmppath;
