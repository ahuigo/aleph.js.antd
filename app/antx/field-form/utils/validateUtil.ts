import RawAsyncValidator from '../../async-validator/index.ts';
...
async function validateRule(
  ...
  try {
    ...
    if (errObj.errors) {
      result = errObj.errors.map(({ message }:{message: any}, index: number) => {
    ...
    .map((currentRule, ruleIndex) => {
      ...
      if (originValidatorFunc) {
        cloneRule.validator = (
          ...
          // Get promise
          // deno-lint-ignore
          const promise = originValidatorFunc(rule, val,
          // eslint-disable-next-line
            wrappedCallback as ((arg?: string) => void),
          );
          hasPromise =
...
async function finishOnAllFailed(rulePromises: Promise<RuleError>[]): Promise<RuleError[]> {
  return Promise.all(rulePromises).then(
    (errorsList: RuleError[]): RuleError[] | Promise<RuleError[]> => {
      const errors: RuleError[] = [...errorsList];
