import get from '../../rc-util/utils/get.ts';
import set from '../../rc-util/utils/set.ts';
import type { InternalNamePath, NamePath, Store, StoreValue, EventArgs } from '../interface.tsx';
...
function internalSetValues<T>(store: T, values: T): T {
  ...
  Object.keys(values).forEach((key) => {
    type Key = keyof typeof newStore
    const prevValue = newStore[key as Key];
    const value = values[key  as Key];

    // If both are object (but target is not array), we use recursion to set deep value
    const recursive = isObject(prevValue) && isObject(value);

    newStore[key as Key] = recursive ? internalSetValues(prevValue, value || {}) : cloneDeep(value); // Clone deep for arrays
  });
